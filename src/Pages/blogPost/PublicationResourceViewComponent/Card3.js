        ...
        <Container>
          <h1 style={{ fontWeight: "bold" }}>
            ...
            {blogsSection && blogsSection.filter(
                ...
                )[0]? blogsSection.filter(item=> item.id.toString() === postId.toString())[0].title : null}

            </h1>
            {blogsSection && blogsSection.filter(
                  (item) => item.id.toString() === postId.toString()
                )[0]?<Editor editorState={EditorState.createWithContent(convertFromRaw(JSON.parse(blogsSection.filter(item=> item.id.toString() === postId.toString())[0].detail)))} toolbarClassName='hideToolbar' readOnly/>:null}

          {/* <p
            style={{ fontWeight: "bold", fontSize: "18px", marginTop: "25px" }}
          >
            {blogsData? blogsData.detail : null} */}
