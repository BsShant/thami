...
const Card3 = (props) => {
  ...
  //   articlesData? ()=>EditorState.createWithContent(convertFromRaw(JSON.parse(articlesData.detail))): () => EditorState.createEmpty()
  //  )
  return (
    <React.Fragment>
      <div className="text-center articleBox">
        <Container>
          <h1 style={{ fontWeight: "bold" }}>
            ...
            {/* {articlesData?articlesData.title : null} */}
            {ourArticlesSection && ourArticlesSection.filter(item => item.id.toString() === postId.toString())[0]? ourArticlesSection.filter(item=> item.id.toString() === postId.toString())[0].title : null}

            </h1>
            {ourArticlesSection && ourArticlesSection.filter(item => item.id.toString() === postId.toString())[0]?<Editor editorState={EditorState.createWithContent(convertFromRaw(JSON.parse(ourArticlesSection.filter(item=> item.id.toString() === postId.toString())[0].detail)))} toolbarClassName='hideToolbar' readOnly/>:null}

          {/* <p
            style={{ fontWeight: "bold", fontSize: "18px", marginTop: "25px" }}
          >
            {articlesData? articlesData.detail : null} */}
