...
const Card3 = (props) => {
  ...
  const pressReleaseSection = useSelector(state=>state.publicationStore.pressReleaseSection)

  
  // const [editorState, setEditorState] = React.useState(
  //   pressData? ()=>EditorState.createWithContent(convertFromRaw(JSON.parse(pressData.detail))): () => EditorState.createEmpty()
  //  )
  return (
    <React.Fragment>
      <div className="text-center articleBox">
        <Container>
          <h1 style={{ fontWeight: "bold" }}>
            ...
            {/* {pressData?pressData.title : null} */}
            {pressReleaseSection && pressReleaseSection.filter(
                  (item) => item.id.toString() === postId.toString()
                )[0]? pressReleaseSection.filter(item=> item.id.toString() === postId.toString())[0].title : null}


            </h1>
            {pressReleaseSection && pressReleaseSection.filter(
                  (item) => item.id.toString() === postId.toString()
                )[0]?<Editor editorState={EditorState.createWithContent(convertFromRaw(JSON.parse(pressReleaseSection.filter(item=> item.id.toString() === postId.toString())[0].detail)))} toolbarClassName='hideToolbar' readOnly/>:null}

          {/* <p
            style={{ fontWeight: "bold", fontSize: "18px", marginTop: "25px" }}
          >
            {pressData? pressData.detail : null} */}
