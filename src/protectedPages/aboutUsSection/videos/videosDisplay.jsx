...
const OurTeamDisplay = ()=>{
  ...
  const parsedToken = useSelector(state=>state.authStore.userToken)
   ...
   const onDeleteItem = async(id)=>{
    ...
    try {
      const res = await fetch(`${server}/aboutUs/video`, {
        method: 'DELETE', 
  ...
  const columns = [
          ...
          <div className="tableSmall">
            <div className="row tableSmallContainer">
              ...
              <div className="row tableSmallContainer">
                  ...
                  <a href={`${server}/${record.video}`} target='_blank'>{record.video}</a>
    ...
    return(
      ...
      <div className="col-md-12" style={{padding:"0"}}>
        <h3 style={{margin:"30px auto 30px", paddingLeft:"15px"}}>Our Videos Data</h3>
        <Table dataSource={videosSection? videosSection: []} columns={columns} pagination={{ position: ['bottomCenter'] }}/>
       { isModalVisible? <Modal title="" width={"70%"} footer={null} header={null} visible={isModalVisible} onOk={handleOk} onCancel={handleCancel}>
        <VideosForm title='Update Video' value={record} imageId='updateVideo' imageName='teamImage' method="PUT"/>
        </Modal> : null}
        </div>
