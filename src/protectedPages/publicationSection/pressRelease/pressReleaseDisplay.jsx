...
const OurTeamDisplay = ()=>{
  ...
  const parsedToken = useSelector(state=>state.authStore.userToken)
   ...
   const onDeleteItem = async(id)=>{
    ...
    try {
      const res = await fetch(`${server}/publication/pressRelease`, {
        method: 'DELETE', 
  ...
  const columns = [
    ...
    return(
        ...
        <h3 style={{margin:"30px auto 30px", paddingLeft:"15px"}}>Press Release Data</h3>
        <Table dataSource={pressReleaseSection? pressReleaseSection:[]} columns={columns} pagination={{ position: ['bottomCenter'] }}/>
       { isModalVisible? <Modal title="" width={"70%"} footer={null} header={null} visible={isModalVisible} onOk={handleOk} onCancel={handleCancel}>
        <PressReleaseForm title='Update Press Release' value={record} imageId='updatePressImage' method="PUT"/>
        </Modal> : null}
        </div>
