...
const key = 'updatable';
const newServer = `${server}/getInvolved/donate`
...
const OurPartnersForm = (props)=>{
  ...
  let imageHolder= props.value? `${server}/${props.value.logo}` : ImagePlaceholder
    ...
    const onFinish = async(values)=>{
      ...
      if(!selectedImageFile){
        formData.append('filename',props.value.logo)
      }
      else{
    ...
    return(
        ...
        <input id={props.imageId} type="file" name='logo' onChange={imageFileChange} style={{display:"none"}} accept='image/*'/>
        </div>
        </div>
        </div>
        <div className="col-sm-12 col-md-12">
        ...
        <FormItem label='Payment Heading' Textarea='Aextarea' height="70px" name='heading' />
        <FormItem label='Payment Detail' Textarea='Textarea' height='300px' name='detail' />
