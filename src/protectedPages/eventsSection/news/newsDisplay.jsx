...
const OurTeamDisplay = ()=>{
  ...
  const dispatch = useDispatch()
   ...
   const onDeleteItem = async(id)=>{
    ...
    try {
      const res = await fetch(`${server}/event/news`, {
        method: 'DELETE', 
  ...
  const columns = [
    ...
    return(
        ...
        <div className="col-md-12">
        <h3 style={{margin:"30px auto 30px", paddingLeft:"15px"}}>News Data</h3>
        <Table dataSource={newsSection? newsSection:[]} columns={columns} pagination={{ position: ['bottomCenter'] }}/>
       { isModalVisible? <Modal title="" width={"70%"} footer={null} header={null} visible={isModalVisible} onOk={handleOk} onCancel={handleCancel}>
        <NewsForm title='Update News Post' value={record} imageId='TeamUpdateImage' method="PUT"/>
        </Modal> : null}
        </div>
